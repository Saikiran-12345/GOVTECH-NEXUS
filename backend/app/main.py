...
async def add_correlation_id_and_timing(request: Request, call_next):
    ...
    logger.info(f"Started {request.method} {request.url.path} [ReqID: {request_id}]")
    ...
    try:
        ...
        process_time = (time.time() - start_time) * 1000
        logger.error(f"Failed {request.method} {request.url.path} - Exception: {str(exc)} in {process_time:.2f}ms")
        return JSONResponse(
